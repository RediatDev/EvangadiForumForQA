...
const createUser = async (req, res) => {
  ...
  const validateInputs = () => {
    ...
    if (!trimmedUsername || !trimmedFirstname || !trimmedLastname || 
        !trimmedEmail || !trimmedGender || !trimmedCountry || !trimmedPassword) {
    ...
    } else if (trimmedAgreedToTerms !== true && trimmedAgreedToTerms !== false) {
      errors.push("agreed_to_terms must be either true or false.");
    }
...
const userLogIn = async (req, res) => {
  ...
  const userProfileUpdate = async (req, res) => {
    ...
    if (!isAnyFieldProvided) {
      ...
      // Check for valid country (optional: customize as needed)
...
const userProfileDelete = async (req, res) => {
  ...
  try {
      ...
      if (user.role === "1") {
          return res.status(403).json({ errors: ["Admin profiles cannot be deleted."] });
      }
...
const userPasswordResetRequest = async (req, res) => {
  ...
  try {
      ...
      const resetLink = `${process.env.HTTP_STATUS}://${process.env.DB_HOST}:${process.env.PORT}/api/users/userPasswordReset/${user.userId}`;
    ...
    sendEmail();
      ...
      // Respond with a success message
      return res.status(200).json({ message: ["Password updating email has been sent to your email."]});
